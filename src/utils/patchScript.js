...
function wrapMain(filename, sheet, blocks, { needAppend, needData }) {
  ...
  // Append past the used range. data.length IS the row count, so when we already have
  // data in hand we reuse it rather than making a second round-trip to Excel.
  if (needAppend) pre.push(needData ? '    let apR = data.length;' : '    let apR = S.getUsedRange().getRowCount();')
  return (
...
function buildUniqueKeyScript(changes, map, sheet, keyHeader, filename, { withEntityType }) {
  ...
  // needData is always true now: every lookup reads the in-memory used range.
  return fillUsed(wrapMain(filename, sheet, blocks, { needAppend, needData: blocks.length > 0 }), used)
}
