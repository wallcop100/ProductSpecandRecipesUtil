...
/**
 * TemplatePicker — browsable list of templates with Apply buttons.
 *
 * Props:
 *   posRef      — active position ref
 *   activeTags  — current position's tags (string[])
 *   hasRows     — whether the position already has recipe rows
 *   onApply(templateId) — called after template is applied so BuilderScreen
 *                         can track the appliedTemplateId
 */
export default function TemplatePicker({ posRef, activeTags, hasRows, onApply }) {
  ...
  const enriched = useMemo(() => {
    ...
    return templates
      ...
      .map(t => ({
        ...t,
        _tags: parseTags(t.applicable_tags),
        _isMatch: parseTags(t.applicable_tags).length > 0 &&
          parseTags(t.applicable_tags).every(tag => activeTags.includes(tag)),
      }))
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      {pendingApply && (
        <Alert
          variant="warning"
          className="mx-2 mt-2 mb-0 py-2 px-3"
          style={{ fontSize: 12 }}
        >
          <div className="mb-1 fw-semibold">Applying will replace the current recipe.</div>
          <div className="d-flex gap-2">
            <Button size="sm" variant="warning" onClick={() => doApply(pendingApply)}>
              Apply anyway
            </Button>
            <Button size="sm" variant="outline-secondary" onClick={() => setPendingApply(null)}>
              Cancel
            </Button>
          </div>
        </Alert>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '0.5rem' }}>
        ...
        {!posRef && (
          <div className="text-muted small text-center py-3">
            Select a position to apply a template.
          </div>
        )}
    ...
    >
      ...
      {tpl._tags.length > 0 && (
        <div className="d-flex flex-wrap gap-1 mb-2">
          {tpl._tags.map(tag => (
            <Badge key={tag} bg="secondary" style={{ fontSize: 10, fontWeight: 400 }}>
              {tag}
            </Badge>
          ))}
        </div>
